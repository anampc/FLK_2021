...
## 3.1 Temperature impacts on CC

### 3.1.1 Temperature and TA: Model II OLS regressions 

* Tem-TA plots by zone 
...
* Tem-TA plots by region and zone 
...
* Are TA vs Sal slopes different by region?
...
#DIC_Sal+ facet_grid(Zone~Precipitation)
...
* Tem-DIC plots by region and zone 
...
* Are DIC vs Sal slopes different by region?
...
kable(as.data.frame(TA_DIC_Sal[,1:8], format = "markdown"), 
      caption = "Table: TA/DIC vs Tem by zone and region",
      digits = 3)%>% 
      kable_styling(bootstrap_options = c("striped", "condensed", 
                                      full_width = F), font_size = 11)
#write.csv(TA_DIC_Sal, "FLK_results/TA_DIC_all.csv")
...
* Salinity-TA: Model II OLS regressions 
...
DIC_coef_OLS <- FLK.data %>%
  ...
  kable(as.data.frame(DIC_coef_OLS[,1:9], format = "markdown"), 
      caption = "Table: DIC vs salinity model II regressions",
      digits = 3)%>% 
  kable_styling(bootstrap_options = c("striped", "condensed", full_width = F), font_size = 11)
  #write.csv(DIC_coef_OLS, "FLK_results/DIC_Sal.csv")
...
Salinity_Region<-ggarrange(TA_Sal_Reg + facet_grid(~Zone) +
          ...
          #labels = c("A", "B"),
          ncol = 1, nrow = 2, heights = c(1, .9))
Salinity_Region
#ggsave(file="FLK_results/Salinity_region.svg", plot=Salinity_Region, dpi = 300, width=5, height=4)
```

* TA and DIC vs Salinity - region simplified 

```{r, salinity figure, echo=FALSE, fig.height=6, fig.width=8}
Salinity_plots<-ggarrange(TA_Sal + facet_grid(~Zone) +
                            guides(fill = guide_legend(
                              override.aes=list(shape=21)),
                              shape = "none")+
                            theme(axis.title.x=element_blank(),
          ...
          ncol = 1, nrow = 2, heights = c(1, .9))
Salinity_plots
#ggsave(file="FLK_results/Salinity_plots_edited.svg", plot=Salinity_plots, dpi = 300, width=5, height=4)
...
### Figure 4: TA, DIC, TA/DIC vs salinity
...
**Figure Supp X:** Linear regressions of mean salinity normalized total alkalinity (n36TA, µmol kg−1) as a function of measured Total Alcalinity (TA, µmol kg−1)
...
# - Figure 4: Seasonal Δ and yearly sums - RAW DATA
...
**Figure 4 (RAW): Seasonal inshore-ocean gradients in the Florida Keys. (A) Seasonal ΔTA. (B) Seasonal ΔDIC. (C) Seasonal pH, and (D) Seasonal ΩAr.** The bars represent the mean Δ values per season, region, and shelf position (± 95 CI).
...
Trends_Modeled<-ggarrange(
                  Trends_pCO2 + theme(legend.box.background = element_rect(),),
                  Trends_pH,
                  Trends_Omega,
                  Trends_TA,
                  Trends_DIC,
          #labels = c("A", "B", "C"),
          ncol = 1, nrow = 5, heights = c(0.9, 0.9, 0.9, 0.9, 1.1),
