...
Two extreme weather events can be observed in the data and associated with abnormal conditions:
...
# Label and filter extreme events
  ...
  FLK.data$Extreme[FLK.data$Extreme=="Low_pH"]<-"Normal"
  summary(FLK.data$Extreme)
  FLK.data_filtered<-FLK.data[FLK.data$Extreme=="Normal",]
```

# 3 Temperature, salinity and light impacts on carbonate chemistry
...
```{r}
library(lubridate)
light<-read.csv("Data/light.csv", header = T)
light$Time <- hms(light$Time)
light$Time<-hour(light$Time) + minute(light$Time)/60
light$Day<-as.character(light$Day)
summary(light)
light.summary<- ddply (light, .(Month2),
                         summarise,
                        h_light= mean (Time, na.rm = T))

FLK.data<-FLK.data %>%
    mutate(MD = format(as.Date(FLK.data$ESTDate), "%d-%b"))

FLK.data<-join(FLK.data, light, by=c("MD", "Month2"), type="left")
summary(FLK.data$Time)
```
...
TA_light<- ggplot(FLK.data) + MyTheme+
  geom_point(aes (Time, TA_umol_kg, fill=Sub_region, shape=Zone),
             alpha=0.7, size=2)+
  # geom_smooth(aes(Temperature_C, BestSalinity, colour=Sub_region),
  #             linetype=1, method = "lm", se=F)+
  geom_smooth(aes(Time, TA_umol_kg, colour=Sub_region),
              linetype=1, method = "lm", se=F)+
  #Precipitation_fill+
  #Season_fill+
  Zone_shapes4+
  ...
  scale_x_continuous(#limits = c(1700, 2400),
                     expand = c(0.02, 0.02),
                     name=(expression("Day light duration"~(h))))+
 theme(axis.text.x = element_text(angle = 0))
TA_light + facet_grid(~Zone)
```

* Light-Sal: Model II OLS regressions - only zone 

```{r}
L_TA_coef_OLS <- FLK.data %>%
                group_by(Zone) %>%
                do({model = lm(TA_umol_kg~Time, data=.)  # model
                    data.frame(tidy(model), # get coefficient info
                    glance(model))})        # get model info
#L_TA_coef_OLS

kable(as.data.frame(L_TA_coef_OLS[,1:9], format = "markdown"), 
      caption = "Table: TA vs Day duraton linear regressions by zone",
      digits = 3)%>% 
kable_styling(bootstrap_options = c("striped", "condensed", full_width = F), font_size = 11)
...
```{r,  fig.height=4, fig.width=8}
DIC_light<- ggplot(FLK.data) + MyTheme+
  geom_point(aes (Time, DIC_umol_kg, fill=Season, shape=Zone),
             alpha=0.7, size=2)+
  # geom_smooth(aes(Temperature_C, BestSalinity, colour=Sub_region),
  #             linetype=1, method = "lm", se=F)+
  geom_smooth(aes(Time, DIC_umol_kg, colour=Sub_region),
              linetype=1, method = "lm", se=F)+
  #Precipitation_fill+
  Season_fill+
  Zone_shapes4+
  ...
  scale_x_continuous(#limits = c(1700, 2400),
                     ...
                     name=(expression("Day light duration"~(h))))+
 theme(axis.text.x = element_text(angle = 0))
DIC_light + facet_grid(Sub_region~Zone)
```
...
TA_DIC_Tem<- ggplot(FLK.data) + MyTheme+
...
DIC_Sal<- ggplot(FLK.data) + MyTheme+
  ...
  geom_smooth(aes(BestSalinity, DIC_umol_kg),
              linetype=1, method = "lm", se=F, color="black")+
  Season_fill+
  guides(shape="none")+
  Zone_shapes4+
  scale_y_continuous(#limits = c(1700, 2400),
...
TA_DIC_Sal<- ggplot(FLK.data) + MyTheme+
  ...
  #             linetype=1, method = "lm", se=F)+
   geom_smooth(aes(BestSalinity, TA_DIC),
               linetype=1, method = "lm", se=F, colour="black")+
  Zone_shapes4+
  Season_fill+
  guides(shape="none")+
  scale_y_continuous(#limits = c(1700, 2400),
...
## Figure 5: Summary by zone and season
...
Zone_Plot
#ggsave(file="FLK_results/Figures/Fig_4/Sal_Te_Zone.svg", plot=Zone_Plot, dpi = 300, width=8, height=5)
```

# - Figure 5: Summary salinity plots with region
...
write_csv(TA_DIC_coef_Season_noExtremes, "Table_S16.csv")
...
Slope_plot<-ggplot(data=Slopes_season, aes (x =Zone, y =estimate)) +
  MyTheme+
  Zone_shapes4+
  stat_summary(aes (x=Zone,
                    y=estimate, colour=Season),
...
## 4.4 By Season - nTA and nDIC


```{r}
# Individual LR for each Zone and Sub_region
nTA_nDIC_coef_zone_noExtremes <- No_ocean.data[No_ocean.data$Extreme=="Normal", ] %>% 
                    group_by(Zone, Sub_region) %>%
...
kable(as.data.frame(nTA_nDIC_coef_zone_noExtremes[,1:8], 
      ...
      digits = 2)%>% 
kable_styling(bootstrap_options = c("striped", "condensed", full_width = F), font_size = 11)
#write.csv(nTA_nDIC_coef_zone_noExtremes, "FLK_results/Fig_2_TA_DIC_coef_slopes_Subregion.csv", row.names = FALSE)

# Annotations for plots
  coef_zone_noExtremes<-dplyr::select(nTA_nDIC_coef_zone_noExtremes, 
                               c("Zone", "Sub_region", "estimate", "r.squared"))
  coef_zone_noExtremes$estimate<-round(coef_zone_noExtremes$estimate, digits=2)
  coef_zone_noExtremes$r.squared<-round(coef_zone_noExtremes$r.squared, digits=2)
```
...
nTA_DIC_season<-TA_DIC + 
  Season_colour+ Season_fill+
  geom_abline(intercept = -1700, slope=2, color="grey")+
  geom_hline (yintercept = TA_DIC_Ocean$nTAmean, color="grey")+
  scale_shape_manual(values=c(24, 25, 23, 22, 21))+
  geom_point(data=No_ocean.data, aes (nDIC, nTA,
                                 fill=Season, shape=Extreme),
                            alpha=0.5, size=1)+
  
   geom_smooth(data=No_ocean.data[No_ocean.data$Extreme=="Normal", ], 
              aes(nDIC, nTA, colour=Season), 
              method = "lm", se=F, size=0.5)+
  geom_smooth(data=No_ocean.data[No_ocean.data$Extreme=="Normal", ], 
              aes(nDIC, nTA), 
              method = "lm", colour="black", se=F)+
  ...
  geom_text(data = coef_zone_noExtremes, 
            aes(x = TA_DIC_Ocean$nDICmean, y = 2070,
                label = paste("Slo =", estimate,  
                              " r.sq=", r.squared)),
            size=2.5)+
...
```{r}
# Individual LR for each reef
nTA_DIC_coef_Season_noExtremes <- FLK.data[FLK.data$Extreme=="Normal", ] %>% 
                    group_by(Zone, Sub_region, Season) %>%
                    do({model = lm(nTA~nDIC, data=.)  
                    # create model
                    data.frame(tidy(model), # get coefficient info
...
nTA_DIC_coef_Season_noExtremes<-subset(nTA_DIC_coef_Season_noExtremes, term!="(Intercept)")
kable(as.data.frame(nTA_DIC_coef_Season_noExtremes[,1:11],
                      format = "markdown"),
        caption = "TA vs DIC equations by season without extreme events",
        digits = 2)%>% 
kable_styling(bootstrap_options = c("striped", "condensed", full_width = F), font_size = 11)
#write.csv (nTA_DIC_coef_Season_noExtremes, "FLK_results/Fig_5_TA_DIC_slopes_Reefs.csv", row.names = FALSE)
...
```{r, fig.height=8, fig.width=4}
nSlopes_season<-subset(nTA_DIC_coef_Season_noExtremes, term!="(Intercept)")

nSlope_plot<-ggplot(data=nSlopes_season, aes (x =Zone, y =estimate)) +
  MyTheme+
  Zone_shapes4+
  stat_summary(aes (x=Zone,
                    y=estimate, colour=Season),
...
```{r,  echo=FALSE}
FLK_slopes<-ggarrange(TA_DIC_season, Slope_plot, nSlope_plot,
          #labels = c("A", "B"),
          ncol = 3, nrow = 1, widths = c(4, 1, 1))
FLK_slopes

FLK_slopes<-ggarrange(Slope_plot, nSlope_plot,
          #labels = c("A", "B"),
...
## 4.5 By Season - n35TA and n35DIC


```{r}
# Individual LR for each Zone and Sub_region
n35TA_n35DIC_coef_zone_noExtremes <- No_ocean.data[No_ocean.data$Extreme=="Normal", ] %>% 
                    group_by(Zone, Sub_region) %>%
                    do({model = lm(n36TA ~n36DIC, data=.)  
                    # create model
                    data.frame(tidy(model), # get coefficient info
                    glance(model))})        # get model info

n35TA_n35DIC_coef_zone_noExtremes<-subset(n35TA_n35DIC_coef_zone_noExtremes, term!="(Intercept)")

kable(as.data.frame(n35TA_n35DIC_coef_zone_noExtremes[,1:8], 
      format = "markdown"),
      caption = "n36TA vs n36DIC equations by zone and region without extreme events",
      digits = 2)%>% 
kable_styling(bootstrap_options = c("striped", "condensed", full_width = F), font_size = 11)
#write.csv(n35TA_n35DIC_coef_zone_noExtremes, "FLK_results/Fig_2_TA_DIC_coef_slopes_Subregion.csv", row.names = FALSE)

# Annotations for plots
  n36coef_zone_noExtremes<-dplyr::select(n35TA_n35DIC_coef_zone_noExtremes, 
                               c("Zone", "Sub_region", "estimate", "r.squared"))
  n36coef_zone_noExtremes$estimate<-round(n36coef_zone_noExtremes$estimate, digits=2)
  n36coef_zone_noExtremes$r.squared<-round(n36coef_zone_noExtremes$r.squared, digits=2)
```

```{r, fig.height=8, fig.width=8.8}

n36TA_DIC_season<-TA_DIC + 
  Season_colour+ Season_fill+
  geom_abline(intercept = -1700, slope=2, color="grey")+
  geom_hline (yintercept = TA_DIC_Ocean$n36TAmean, color="grey")+
  scale_shape_manual(values=c(24, 25, 23, 22, 21))+
  geom_point(data=No_ocean.data, aes (n36DIC, n36TA,
                                 fill=Season, shape=Extreme),
                            alpha=0.5, size=1)+
  
   geom_smooth(data=No_ocean.data[No_ocean.data$Extreme=="Normal", ], 
              aes(n36DIC, n36TA, colour=Season), 
              method = "lm", se=F, size=0.5)+
  geom_smooth(data=No_ocean.data[No_ocean.data$Extreme=="Normal", ], 
              aes(n36DIC, n36TA), 
              method = "lm", colour="black", se=F)+
  
  geom_text(data = n36coef_zone_noExtremes, 
            aes(x = TA_DIC_Ocean$n36DICmean, y = 2070,
                label = paste("Slo =", estimate,  
                              " r.sq=", r.squared)),
            size=2.5)+

  geom_point(aes(x=TA_DIC_Ocean$n36DICmean, y=TA_DIC_Ocean$n36TAmean),
             size=3, alpha=1, shape=21, fill="black")
#n36TA_DIC_season
...
```{r}
# Individual LR for each reef
n36TA_n36DIC_coef_Season_noExtremes <- FLK.data[FLK.data$Extreme=="Normal", ] %>% 
                    group_by(Zone, Sub_region, Season) %>%
                    do({model = lm(n36TA~n36DIC, data=.)  
                    # create model
                    data.frame(tidy(model), # get coefficient info
...
n36TA_n36DIC_coef_Season_noExtremes<-subset(n36TA_n36DIC_coef_Season_noExtremes, term!="(Intercept)")
kable(as.data.frame(n36TA_n36DIC_coef_Season_noExtremes[,1:11],
                      format = "markdown"),
        caption = "n36TA vs n36DIC equations by season without extreme events",
        digits = 2)%>% 
kable_styling(bootstrap_options = c("striped", "condensed", full_width = F), font_size = 11)
#write.csv (n36TA_n36DIC_coef_Season_noExtremes, "FLK_results/Fig_5_TA_DIC_slopes_Reefs.csv", row.names = FALSE)
...
```{r, fig.height=8, fig.width=4}
n36Slopes_season<-subset(n36TA_n36DIC_coef_Season_noExtremes, term!="(Intercept)")

n36Slope_plot<-ggplot(data=n36Slopes_season, aes (x =Zone, y =estimate)) +
  MyTheme+
  Zone_shapes4+
  stat_summary(aes (x=Zone,
                    y=estimate, colour=Season),
  ...
  geom_point(aes(fill = factor(Season)),
             ...
             shape = 21, size=0.5)+
  scale_y_continuous(limits = c(0,1.2),
                     expression(paste("n36TA vs n36DIC slope")))+
  facet_grid(Sub_region~Region)+
  Season_fill+
  Season_colour+
  guides(fill=FALSE)+
...
```{r,  echo=FALSE}
FLK_slopes<-ggarrange(Slope_plot, nSlope_plot, n36Slope_plot,
          #labels = c("A", "B"),
...
Month_Season_plot_in<- ggplot(FLK.data_filtered[
          FLK.data_filtered$Zone!="Oceanic",]) +
        MyTheme +  
        facet_grid(~Zone)+
        guides(fill=guide_legend(override.aes=list(shape=21)))+
...
### Figure 3C - nTA (not used)

```{r}
nTA_exp<- Month_Season_plot+
   geom_abline(slope = 0, intercept = mean_Op$OTA, color="grey")+
   scale_y_continuous(#limits = c(2100,2500),
                     expand = c(0.05, 0.05),
                     breaks = seq(2100, 2500, 100),
  ...
  #               aes(x=as.numeric(Month2), y=Temp.adj))+
    ...
    stat_summary(aes (x=as.numeric(Month2),
                ...
                position=position_dodge(width=0.3))+
    stat_summary(aes (x=as.numeric(Month2), 
                      y=nTA, fill=Sub_region, shape=Zone),
                size=1,
                fun.data = "mean_cl_boot", geom = "point",
                position=position_dodge(width=0.3))
...
nTA_Y3<- Year_plot_3+
   ...
   geom_point(data=Seasonal_range, aes (x=Zone, y=nTA_max,
                   shape=Zone, colour=Sub_region),
              alpha=0.8, size=1,
              position = position_dodge(width = 0.6))+
   geom_point(data=Seasonal_range, aes (x=Zone, y=nTA_min,
                   shape=Zone, colour=Sub_region),
              alpha=0.8, size=1,
              position = position_dodge(width = 0.6))+
   # geom_segment(data=Seasonal_range, 
   ...
   #              #position = position_dodge(width = 0.6), 
   #              alpha=0.3)+
   stat_summary(aes (x=Zone, y=nTA_mean, colour=Sub_region),
               fun.data = "mean_sdl", fun.args = list(mult = 1),
                geom = "errorbar",  width = 0.2,
                position = position_dodge(width = 0.6))+
   stat_summary(aes (x=Zone, y=nTA_mean, fill=Sub_region, shape=Zone),
                fun = "mean", 
                geom = "point", size=2,
                position = position_dodge(width = 0.6))
#nTA_Y3
...
```{r, echo=FALSE, fig.height=3, fig.width=8}
nTA_my2<-ggarrange(#TA_exp, TA_Y2 + theme(axis.title.y = element_blank()),
          nTA_exp + theme(legend.position = "none",
                         strip.text.x = element_blank(),
                         axis.text.x = element_blank()),
          nTA_Y3 + theme(legend.position = "none",
                        strip.text.x = element_blank(),
                        axis.title.x = element_blank(),
                        axis.title.y = element_blank(),
                        axis.text.x = element_blank()),
...
# 2. and subtract them from the reef data
  ...
  WS.reef$dOme_MY<-WS.reef$Aragonite_Sat_W-WS.reef$OAra_O
    # Delta Temp
  WS.reef$dTem_MY<-WS.reef$Temperature_C -WS.reef$OTem_my
   # Delta Sal
  WS.reef$dSal_MY<-WS.reef$BestSalinity-WS.reef$OSal_my
...
Dive dataset by Zone to run models
...
# - Figure 5: Seasonal delta models
...
# 6 Seasonal sensitivity analysis

## 6.1 Calculate partial derivatives 

```{r}
library(seacarb)

# Derivatives with respect of TA
    der.TA = derivnum(
      varid = "var1",
      flag = 15,
      ...
      T = mean_Op$OTem,
      P = 0,
      Patm=1.0, Pt=0, Sit=0, pHscale="T"
      )
    Arg_der.TA<-as.numeric(der.TA$OmegaAragonite/ 10 ^ 6)
    pH_der.TA<-as.numeric(der.TA$pH/ 10 ^ 6)
    pCO2_der.TA<-as.numeric(der.TA$pCO2/ 10 ^ 6)
    
    # Derivatives with respect of DIA
    der.DIC = derivnum(
      varid = "var2",
      flag = 15,
      var1 = mean_Op$OTA / 10 ^ 6, #mean oceanic TA
    ...
    Arg_der.DIC<-as.numeric(der.DIC$OmegaAragonite/ 10 ^ 6)
    pH_der.DIC<-as.numeric(der.DIC$pH/ 10 ^ 6)
    pCO2_der.DIC<-as.numeric(der.DIC$pCO2/ 10 ^ 6)
    
  # Derivatives with respect of Temperature (C)
      der.T = derivnum(
        varid = "temp",
        flag = 15,
        var1 = mean_Op$OTA / 10 ^ 6, #mean oceanic TA
    ...
    Arg_der.Temp<-as.numeric(der.T$OmegaAragonite)
    pH_der.Temp<-as.numeric(der.T$pH)
    pCO2_der.Temp<-as.numeric(der.T$pCO2)
      
  # Derivatives with respect of Salinity (psu)
      der.S = derivnum(
        varid = "sal",
        flag = 15,
        var1 = mean_Op$OTA / 10 ^ 6, #mean oceanic TA
    ...
    Arg_der.Sal<-as.numeric(der.S$OmegaAragonite)
    pH_der.Sal<-as.numeric(der.S$pH)
    pCO2_der.Sal<-as.numeric(der.S$pCO2)
```

## 6.2 Calculate omega and pH variability related to TA, DIC, T and S

```{r}
FLK.data_filtered$Arg_TA<-(FLK.data_filtered$TA_umol_kg-mean_Op$OTA)*Arg_der.TA
FLK.data_filtered$Arg_DIC<-(FLK.data_filtered$DIC_umol_kg-mean_Op$ODIC)*Arg_der.DIC
FLK.data_filtered$Arg_Temp<-(FLK.data_filtered$Temperature_C - mean_Op$OTem)*Arg_der.Temp
FLK.data_filtered$Arg_Sal<-(FLK.data_filtered$BestSalinity - mean_Op$OSal)*Arg_der.Sal

FLK.data_filtered$pH_TA<-(FLK.data_filtered$TA_umol_kg- mean_Op$OTA)*pH_der.TA
FLK.data_filtered$pH_DIC<-(FLK.data_filtered$DIC_umol_kg- mean_Op$ODIC)*pH_der.DIC
FLK.data_filtered$pH_Temp<-(FLK.data_filtered$Temperature_C-mean_Op$OTem)*pH_der.Temp
FLK.data_filtered$pH_Sal<-(FLK.data_filtered$BestSalinity -mean_Op$OSal)*pH_der.Sal
```

## 6.3 Plot sensitivity


**Sensitivity plots by region and zone** 

* Individual contributions by region, zone and variable to Omega

```{r}

colors <- c("TA" = "#9e9ac8",
            "DIC" = "#a6d98a",
            "Temperature" = "#fb3a4a95",
            "Salinity" = "#feb24c93",
            "Combined"= "gray",
            "Calculated"="black")

Cont_arg_plot<- ggplot(FLK.data_filtered) +
        MyTheme +  
        facet_grid(Sub_region~Zone)+
        geom_hline(yintercept = 0, colour="gray")+
        scale_color_manual(values = colors)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=Arg_TA, color="TA"),
                span=0.8, se=F)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=Arg_DIC, color="DIC"),
                span=0.8, se=F)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=Arg_Temp, color="Temperature"),
                span=0.8, se=F)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=Arg_Sal, color="Salinity"),
                span=0.8, se=F)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=(Arg_TA+Arg_DIC+Arg_Temp+Arg_Sal), color="Combined"),
                span=0.8, se=F)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=(FLK.data_filtered$Aragonite_Sat_W-mean_Op$OAra),
                      color="Calculated"),
                span=0.8, se=F, linetype=3)+
        scale_y_continuous(#limits = c(-2.5,2.5),
                       name=(expression(
                         Contribution~to~Delta[oc]~Omega[Ar])),
                       breaks = seq(-2, 2, 0.5),  
                       expand = c(0.05, 0.05))+
...
Cont_pH_plot<- ggplot(FLK.data_filtered) +
        MyTheme +  
        facet_grid(Sub_region~Zone)+
        scale_color_manual(values = colors)+
        geom_hline(yintercept = 0, colour="gray")+
        ...
        geom_smooth(aes (x=as.numeric(Months),
                ...
                span=0.8, se=F)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=pH_Temp, color="Temperature"),
                span=0.8, se=F)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=pH_Sal,  color="Salinity"),
                span=0.8, se=F)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=(pH_TA+pH_DIC+pH_Temp+pH_Sal), color="Combined"),
                span=0.8, se=F)+
        geom_smooth(aes (x=as.numeric(Months),
                      y=(FLK.data_filtered$pH_calculated-mean_Op$OpH), 
                      color="Calculated"),
                span=0.8, se=F, linetype=3)+
        scale_y_continuous(#limits = c(-2.5,2.5),
                       name=(expression(
                         Contribution~to~Delta[oc]~pH)),
                       breaks = seq(-2, 2, 0.1),  
                       expand = c(0.01, 0.01))+
        scale_x_continuous(breaks = seq(1, 12, 1),  
                       expand = c(0.01, 0.01))+
...
* TA+DIC and S+T contributions by region, zone and variable to omega

```{r}
Cont_arg_plot_TA_DIC<- ggplot(FLK.data_filtered, aes(x=Months,
                                                     y=(Arg_TA+Arg_DIC),
                                                        fill=Sub_region)) +
        MyTheme +  
        facet_grid(~Zone)+
        ...
        geom_hline(yintercept = 0, colour="gray")+
        geom_smooth(aes (x=as.numeric(Months),
                      y=(Arg_TA+Arg_DIC), colour=Sub_region),
                span=0.8, alpha=0.2, se=F)+
        stat_summary(aes (x=as.numeric(Months),
                    ...
                    position=position_dodge(width=0.3))+
        stat_summary(aes (x=as.numeric(Months), 
                          y=(Arg_TA+Arg_DIC), fill=Sub_region, shape=Zone),
                    size=1,
                fun.data = "mean_cl_boot", geom = "point",
                position=position_dodge(width=0.3))+
         scale_y_continuous(#limits = c(-2.5,2.5),
                       name=(expression(
                        (delta~Omega[Ar]~delta~AT^-1)~Delta~TA~+~
                        (delta~Omega[Ar]~delta~DIC^-1)~Delta~DIC)),
                       breaks = seq(-2, 2, 0.5),  
                       expand = c(0.01, 0.01))+
        scale_x_continuous(breaks = seq(1, 12, 1),  
                       expand = c(0.01, 0.01))
Cont_arg_plot_TA_DIC

Cont_arg_plot_T_S<- ggplot(FLK.data_filtered, aes(x=Months,
                                                     y=(Arg_Temp+Arg_Sal),
                                                        fill=Sub_region)) +
        MyTheme +  
        facet_grid(~Zone)+
        ...
        geom_hline(yintercept = 0, colour="gray")+
        geom_smooth(aes (x=as.numeric(Months),
                      y=(Arg_Temp+Arg_Sal), colour=Sub_region),
                span=0.8, alpha=0.2, se=F)+
        stat_summary(aes (x=as.numeric(Months),
                          y=(Arg_Temp+Arg_Sal), colour=Sub_region),
                    fun.data = "mean_cl_boot", geom = "errorbar", 
                    ...
                    position=position_dodge(width=0.3))+
        stat_summary(aes (x=as.numeric(Months), 
                          y=(Arg_Temp+Arg_Sal), fill=Sub_region, shape=Zone),
                    size=1,
                fun.data = "mean_cl_boot", geom = "point",
                position=position_dodge(width=0.3))+
         scale_y_continuous(#limits = c(-2.5,2.5),
                         name=(expression(
                        (delta~Omega[Ar]~delta~S^-1)~Delta~S~+~
                        (delta~Omega[Ar]~delta~T^-1)~Delta~T)),
...
**Sensitivity plots by zone (region not considered)**

* Individual contributions by zone and variable to Omega and pH 
...
### Figure 7: Seasonal panels

```{r}
Sensitivity<-ggarrange(Cont_arg_plot+ theme(axis.title.x=element_blank(),
                                 axis.text.x=element_blank(),
                                 legend.position = "top"),
                                
                  Cont_pH_plot + theme(legend.position = "none",
                                       strip.text.x = element_blank()),
          ncol = 1, nrow = 2, heights  =c(1.25, 1))
Sensitivity
```

# 7 LMER interannual trend MODELS
...
# Add contributions
Trends.data$Arg_TA<- (Trends.data$TA_umol_kg-mean_Op$OTA)*Arg_der.TA
Trends.data$Arg_DIC<-(Trends.data$DIC_umol_kg-mean_Op$ODIC)*Arg_der.DIC
Trends.data$Arg_Tem<-(Trends.data$Temperature_C - mean_Op$OTem)*Arg_der.Temp
Trends.data$Arg_Sal<-(Trends.data$BestSalinity - mean_Op$OSal)*Arg_der.Sal

Trends.data$pH_TA<-(Trends.data$TA_umol_kg-mean_Op$OTA)*pH_der.TA
Trends.data$pH_DIC<-(Trends.data$DIC_umol_kg-mean_Op$ODIC)*pH_der.DIC
Trends.data$pH_Tem<-(Trends.data$Temperature_C - mean_Op$OTem)*pH_der.Temp
Trends.data$pH_Sal<-(Trends.data$BestSalinity - mean_Op$OSal)*pH_der.Sal
#head(Trends.data)

Reef.Trends.data<-WS.reef[WS.reef$Year>2014, ]
# Add contributions
Reef.Trends.data$Arg_TA<-(Reef.Trends.data$TA_umol_kg-mean_Op$OTA)*Arg_der.TA
Reef.Trends.data$Arg_DIC<-(Reef.Trends.data$DIC_umol_kg-mean_Op$ODIC)*Arg_der.DIC
Reef.Trends.data$Arg_Tem<-(Reef.Trends.data$Temperature_C - mean_Op$OTem)*Arg_der.Temp
Reef.Trends.data$Arg_Sal<-(Reef.Trends.data$BestSalinity - mean_Op$OSal)*Arg_der.Sal

Trends.data$pH_TA<-(Trends.data$TA_umol_kg-mean_Op$OTA)*pH_der.TA
Trends.data$pH_DIC<-(Trends.data$DIC_umol_kg-mean_Op$ODIC)*pH_der.DIC
Trends.data$pH_Tem<-(Trends.data$Temperature_C - mean_Op$OTem)*pH_der.Temp
Trends.data$pH_Sal<-(Trends.data$BestSalinity - mean_Op$OSal)*pH_der.Sal
#summary(Reef.Trends.data)
...
There are not significant changes in mean Temperature over the years
...
In summary, temperature slope is only significant for offshore and zone.
...
TA_all_1<-lmer(TA_umol_kg ~ Year + Zone + Sub_region + 
                  (1 | Month) + (1 | SiteID) + 
                  Zone:Sub_region, data=Trends.data)
  ...
  lstrends(TA_all_1, "Zone", var="Year")
  
In_Newdata<-expand.grid(Sub_region=unique(Trends.data$Sub_region),
                        Zone=unique(Trends.data$Zone),
                        Year=seq((min(Trends.data$Year)),
                                   (max(Trends.data$Year))))

In_TA_Pred <- predict(TA_all_1, newdata=In_Newdata, 
...
# Inshore TA
  ...
  TA_in<-lmer(TA_umol_kg ~ Year + Sub_region +
               (1 | Month) + (1 | SiteID),
                            data=Trends.inshore)
    anova(TA_in)
    ranova(TA_in)
...
# Create a new data frame for independent variables  
  In_Newdata<-expand.grid(Sub_region=unique(Trends.inshore$Sub_region),
                          Year=seq((min(Trends.inshore$Year)-0.2),
                                   (max(Trends.inshore$Year)+0.8)))
...
# Model summary for labels
  slope<-round(summary(TA_in)$coefficients["Year", "Estimate"], digits = 3)
  ys <- max(Trends.midchanel$TA_umol_kg, na.rm =T)
...
TA_mc<-lmer(TA_umol_kg ~ Year + (1 | Month) + (1|SiteID),
               data = Trends.midchanel)
  summary(TA_mc) 
  anova(TA_mc)
...
# Create a new data frame for independent variables  
  Mc_Newdata<-expand.grid(Sub_region=unique(Trends.midchanel$Sub_region),
                          Year=seq((min(Trends.midchanel$Year)-0.2),
                                   (max(Trends.midchanel$Year)+0.8)))
...
# Model summary for labels
  slope<-round(summary(TA_mc)$coefficients["Year", "Estimate"], digits = 3)
  ys <- max(Trends.midchanel$TA_umol_kg, na.rm =T)
  plot_label <- sprintf("\"\" ~ TA == %0.3f", slope)
  plot_label2 <- expression("Year^-1") 
...
TA_of<-lmer(TA_umol_kg ~ Year + (1 | Month), 
               data =Trends.offshore)
  summary(TA_of)
  anova(TA_of) 
...
# Create a new data frame for independent variables  
  Of_Newdata<-expand.grid(one=unique(Trends.offshore$Zone),
                          ...
                          Year=seq((min(Trends.offshore$Year)-0.2), 
                                   (max(Trends.offshore$Year)+0.8)))
...
# Create a new data frame for independent variables  
  Oc_Newdata<-expand.grid(one=unique(Trends.ocean$Zone),
                          ...
                          Year=seq((min(Trends.ocean$Year)-0.2),
                                   (max(Trends.ocean$Year)+0.8)))
...
DIC_all_1<-lmer(DIC_umol_kg ~ Year + Zone + Sub_region + 
  ...
  lstrends(DIC_all_1, "Zone", var="Year")
```
...
# Model summary for labels
  slope<-round(
      summary(Omega_in)$coefficients["Year", "Estimate"], digits = 4)
...
pCO2_in<-lmer(pCO2_uatm ~ Year + (1|Month)+(1|SiteID), data=Trends.inshore)
  summary(pCO2_in)
  anova(pCO2_in)
  step(pCO2_in)
...
anova(pH_all_o, pH_all_o_1, pH_all_o_3)
  ...
  Newdata<-expand.grid(Zone=unique(Trends.data$Zone),
                          ...
                          Year=seq((min(Trends.data$Year)-0), 
                                   (max(Trends.data$Year)+1)))
...
pH_in<-lmer(pH_calculated ~ Year+ Sub_region + (1 | Month) + (1 | SiteID),
            data=Trends.inshore)
  summary(pH_in)    
  anova(pH_in)
  plot(pH_in)
...
#Model summary for slope labels
  slope<-round(
      summary(pH_in)$coefficients["Year", "Estimate"], digits = 4)
  #xs <- as.Date(as.character("2018-11-20"))
...
```{r, echo=FALSE, fig.height=4, fig.width=8}
hist(Trends.data$nTA)
shapiro.test(Trends.data$nTA) # Normality rejected
  
TA_all<- Long_plot+
    scale_y_continuous(#limits = c(15,40),
                       name=(expression(paste("nTA (", mu, "mol ",
                                              kg^-1, ")"))),
                       #breaks = seq(0, 10, 0.5),  
                       expand = c(0.05, 0.05)) +
    geom_point(aes (x=Date, y=nTA, fill=Sub_region), 
             alpha=0.8, size=0.8, shape=21)

TA_all+
    geom_smooth(aes (x=Date, y=nTA), method = "lm",
              color="black") + scale_fill_hue(l=100)
...
```{r, echo=TRUE, fig.height=4, fig.width=6}
## Model with all data
TA_all_o<-lmer(nTA~Year * Zone * Sub_region + (1|Month)+           
                                    (1|SiteID), data=Trends.data)
  step(TA_all_o)
    
    
TA_all_1<-lmer(nTA ~ Year + Zone + Sub_region + 
                  (1 | Month) + (1 | SiteID) + 
                  Zone:Sub_region, data=Trends.data)
...
```{r, fig.height=4, fig.width=6}
# Inshore TA
  hist(Trends.inshore$nTA)
  shapiro.test(Trends.inshore$nTA) # Normality NOT rejected

  TA_in_0<-lmer(nTA~Year * Sub_region + (1|Month)+
                                    #(1|Zone)+             
                                    (1|SiteID), data=Trends.inshore)
  #summary(TA_in_0)
  step(TA_in_0)
    
  TA_in<-lmer(nTA ~ Year + Sub_region +
               (1 | Month) + (1 | SiteID),
                            data=Trends.inshore)
    anova(TA_in)
    ranova(TA_in)
...
# Model summary for labels
  slope<-round(summary(TA_in)$coefficients["Year", "Estimate"], digits = 3)
  ys <- max(Trends.midchanel$nTA, na.rm =T)
  plot_label <- sprintf("\"\" ~ TA == %0.3f", slope)
  plot_label2 <- expression("Year^-1")  
...
```{r, fig.height=4, fig.width=6}
hist(Trends.midchanel$nTA)
shapiro.test(Trends.midchanel$nTA)# Normality rejected

TA_mc_0<-lmer(nTA~Year * Sub_region + (1|Month) + 
                                  #(1|Zone)+             
                                  (1|SiteID), data=Trends.midchanel)
  anova(TA_mc_0)
  ranova(TA_mc_0)
...
TA_mc<-lmer(nTA ~ Year + (1 | Month) + (1|SiteID),
               data = Trends.midchanel)
  summary(TA_mc) 
  anova(TA_mc)
...
```{r, fig.height=4, fig.width=6}
hist(Trends.offshore$nTA)
shapiro.test(Trends.offshore$TA_umol_kg)# Normality rejected

TA_of_0<-lmer(nTA~Year * Sub_region + 
               (1|Month) + (1|SiteID),
               data=Trends.offshore)
  #summary(TA_of_0)    
  anova(TA_of_0)
...
TA_of<-lmer(nTA ~ Year + Sub_region + (1 | Month), 
               data =Trends.offshore)
  summary(TA_of)
  anova(TA_of) 
...
# Create a new data frame for independent variables  
  ...
  #xs <- as.Date(as.character("2018-11-20"))
  #ys <- max(Trends.inshore$TA_umol_kg, na.rm =T)
  ys <- max(Trends.offshore$nTA, na.rm =T)
  plot_label <- sprintf("\"\" ~ TA == %0.3f", slope)
  plot_label2 <- expression("Year^-1")
  
# Plot raw data point and fitted model 
  Of_nTA_Modeled<-Long_plot2+
     scale_y_continuous(#limits = c(2250,2475),
                       name=(expression(paste("nTA (", mu, "mol ",
                                              kg^-1, ")"))),
                       #breaks = seq(0, 10, 0.5),  
...
```{r, fig.height=4, fig.width=6}
hist(Trends.ocean$nTA)
shapiro.test(Trends.ocean$TA_umol_kg)# Normality NOT rejected

TA_oc_0<-lmer(nTA ~ Year * Sub_region + 
               (1 | Month)+ (1|SiteID),
               data=Trends.ocean)
  #summary(TA_oc_0)
  anova(TA_oc_0)
...
TA_oc<-lmer(nTA ~ Year + (1 | Month), data = Trends.ocean)
  summary(TA_oc)
  anova(TA_oc)
...
# Model summary for labels
  slope<-round(summary(TA_oc)$coefficients["Year", "Estimate"], digits = 3)
  #xs <- as.Date(as.character("2018-11-20"))
  #ys <- max(Trends.inshore$nTA, na.rm =T)
  ys <- max(Trends.offshore$nTA, na.rm =T)
  plot_label <- sprintf("\"\" ~ TA == %0.3f", slope)
  plot_label2 <- expression("Year^-1")
...
```{r , all fixed factors summary - nTA}
# ANOVAs
    # Convert ANOVA results into dataframes allows for easier name manipulation
    av.nTA_in_0 <- data.frame (anova(TA_in_0))
    av.nTA_mc_0 <- data.frame (anova(TA_mc_0) )
    av.nTA_of_0 <- data.frame (anova(TA_of_0))
    av.nTA_oc_0 <- data.frame (anova(TA_oc_0))
    
    # Putting all into one dataframe/table
    anova_nTA_0 <- data.frame(cbind
                    (c("Year", "Sub_region", "Year:Sub_region", 
                       "Year", "Sub_region", "Year:Sub_region",
                       "Year", "Sub_region", "Year:Sub_region",
                       "Year", "Sub_region", "Year:Sub_region"), 
                                      rbind(av.nTA_in_0, av.nTA_mc_0,
                                            av.nTA_of_0, av.nTA_oc_0))) 
    colnames(anova_nTA_0) <- c("", "Sum Sq", "Mean Sq", "N Df" , "D Df",
                                     "F value", "Pr(>F)")
    row.names(anova_nTA_0) <- NULL
    
    # create HTML table using kableExtra
    anova_nTA_0 %>% kable("html", digits=2, 
        caption="Initial nTA models fixed factors: Year * Region") %>% 
      kable_styling(bootstrap_options = "striped", full_width = F) %>% 
      pack_rows(., "nTA inshore", 1, 3) %>% # groups rows with label
      pack_rows(., "nTA mid-channel", 4, 6)  %>% # groups rows with label
      pack_rows(., "nTA offshore", 7, 9) %>% # groups rows with label
      pack_rows(., "nTA ocean", 10, 12)  
```
...
# R-ANOVAs    
    ...
    # create HTML table using kableExtra
    ranova_TA_0 %>% kable("html", digits=2,
         caption="Initial nTA models random factors: Month+Station") %>% 
      kable_styling(bootstrap_options = "striped", full_width = F) %>% 
      pack_rows(., "TA inshore", 1, 3) %>% # groups rows with label
      pack_rows(., "TA mid-channel", 4, 6)  %>% # groups rows with label
      pack_rows(., "TA offshore", 7, 9) %>% # groups rows with label
...
modelsummary(TA_models_0, stars = TRUE, 
             #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Initial nTA model outputs (all factors)',
             coef_map=factor_order)
#modelsummary(TA_models, estimate = "p.value")
```
...
modelsummary(TA_models, stars = TRUE, 
            #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Final nTA model outputs (dropped factors)',
             coef_map=factor_order)
...
```{r nTA model plot, fig.height=4, fig.width=8}
Trends_nTA<-ggarrange(In_TA_Modeled+ theme(
                                          axis.text.x=element_blank(),
                                          legend.position = "none"),
                  Mc_TA_Modeled+ theme(legend.position = "none",
                                           axis.text.x=element_blank(),
...
```{r, echo=FALSE, fig.height=4, fig.width=8}
hist(Trends.data$nDIC)
shapiro.test(Trends.data$nDIC) # Normality rejected
...
DIC_all<- Long_plot+
    scale_y_continuous(#limits = c(15,40),
                       ...
                       #breaks = seq(0, 10, 0.5),  
                       expand = c(0.05, 0.05)) +
    geom_point(aes (x=Date, y=nDIC, fill=Sub_region), 
             alpha=0.8, size=0.8, shape=21)

DIC_all+
    geom_smooth(aes (x=Date, y=nDIC), method = "lm",
              color="black") + scale_fill_hue(l=100)

Trends_c<-DIC_all+facet_grid(~Zone) +
  geom_smooth(aes (x=Date, y=nDIC, 
                   colour=Sub_region), 
              method = "lm", alpha=0.3)
Trends_c
...
```{r, echo=TRUE, fig.height=4, fig.width=6}
## Model with all data
DIC_all_o<-lmer(nDIC~Year * Zone * Sub_region + (1|Month)+           
                                    (1|SiteID), data=Trends.data)
  step(DIC_all_o)
    
    
DIC_all_1<-lmer(nDIC ~ Year + Zone + Sub_region + 
                  (1 | Month) + (1 | SiteID) + 
                  Year:Sub_region + Zone:Sub_region, data=Trends.data)
...
```{r, fig.height=4, fig.width=6}
# Inshore DIC
  hist(Trends.inshore$nDIC)
  shapiro.test(Trends.inshore$DIC_umol_kg) # Normality NOT rejected

  DIC_in_0<-lmer(nDIC~Year * Sub_region + (1|Month)+
                                    #(1|Zone)+             
                                    (1|SiteID), data=Trends.inshore)
  ...
  DIC_in<-lmer(nDIC ~ Year + Sub_region +
               (1 | Month) + (1 | SiteID),
                            data=Trends.inshore)
    anova(DIC_in)
    ranova(DIC_in)
...
# Create a new data frame for independent variables  
  ...
  In_DIC_Modeled<-Long_plot2+ 
     scale_y_continuous(limits = c(1700,2320),
                       name=(expression(paste("nDIC (", mu, "mol ",
                                              kg^-1, ")"))),
                       breaks = seq(1700,2320, 200),  
                       expand = c(0.02, 0.02)) +
...
```{r, fig.height=4, fig.width=6}
hist(Trends.midchanel$nDIC)
shapiro.test(Trends.midchanel$DIC_umol_kg)# Normality rejected

DIC_mc_0<-lmer(nDIC~Year * Sub_region + (1|Month) + 
                                  #(1|Zone)+             
                                  (1|SiteID), data=Trends.midchanel)
...
DIC_mc<-lmer(nDIC ~ Year + Sub_region + (1 | Month),
               data = Trends.midchanel)
  summary(DIC_mc) 
  anova(DIC_mc)
...
```{r, fig.height=4, fig.width=6}
hist(Trends.offshore$nDIC)
shapiro.test(Trends.offshore$DIC_umol_kg)# Normality rejected

DIC_of_0<-lmer(nDIC~Year * Sub_region + 
               (1|Month) + (1|SiteID),
               data=Trends.offshore)
#summary(DIC_of_0)    
anova(DIC_of_0)
...
DIC_of<-lmer(nDIC ~ Year + Sub_region + (1 | Month), 
               data =Trends.offshore)
  summary(DIC_of)
  anova(DIC_of) 
...
```{r, fig.height=4, fig.width=6}
hist(Trends.ocean$nDIC)
shapiro.test(Trends.ocean$DIC_umol_kg)# Normality NOT rejected

DIC_oc_0<-lmer(nDIC ~ Year * Sub_region + 
               (1 | Month)+ (1|SiteID),
               data=Trends.ocean)
  #summary(DIC_oc_0)
  anova(DIC_oc_0)
...
DIC_oc<-lmer(nDIC ~ Year + (1 | Month), data = Trends.ocean)
  summary(DIC_oc)
  anova(DIC_oc)
...
```{r , all fixed factors summary - nDIC}
# ANOVAs
    # Convert ANOVA results into dataframes allows for easier name manipulation
    av.DIC_in_0 <- data.frame (anova(DIC_in_0))
    av.DIC_mc_0 <- data.frame (anova(DIC_mc_0) )
    ...
    # create HTML table using kableExtra
    anova_DIC_0 %>% kable("html", digits=2, 
        caption="Initial nDIC models fixed factors: Year * Region") %>% 
      kable_styling(bootstrap_options = "striped", full_width = F) %>% 
      pack_rows(., "DIC inshore", 1, 3) %>% # groups rows with label
      pack_rows(., "DIC mid-channel", 4, 6)  %>% # groups rows with label
      pack_rows(., "DIC offshore", 7, 9) %>% # groups rows with label
...
# R-ANOVAs    
    ...
    ranova_DIC_0 %>% kable("html", digits=2,
         caption="Initial DIC models random factors: Month+Station") %>% 
      kable_styling(bootstrap_options = "striped", full_width = F) %>% 
      pack_rows(., "nDIC inshore", 1, 3) %>% # groups rows with label
      pack_rows(., "nDIC mid-channel", 4, 6)  %>% # groups rows with label
      pack_rows(., "nDIC offshore", 7, 9) %>% # groups rows with label
      pack_rows(., "nDIC ocean", 10, 12)  
...
modelsummary(DIC_models_0, stars = TRUE, 
             #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Initial nDIC model outputs (all factors)',
             coef_map=factor_order)
#modelsummary(DIC_models, estimate = "p.value")
```
...
modelsummary(DIC_models, stars = TRUE, 
            #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Final nDIC model outputs (dropped factors)',
             coef_map=factor_order)
...
```{r nDIC model plot, fig.height=4, fig.width=8}
Trends_nDIC<-ggarrange(In_DIC_Modeled+ theme(
                                          #axis.text.x=element_blank(),
                                          legend.position = "none"),
                  Mc_DIC_Modeled+ theme(legend.position = "none",
                                           #axis.text.x=element_blank(),
...
```{r, echo=FALSE, fig.height=4, fig.width=8}
hist(Trends.data$n36TA)
shapiro.test(Trends.data$n36TA) # Normality rejected
  
TA_all<- Long_plot+
    scale_y_continuous(#limits = c(15,40),
                       name=(expression(paste("n36TA (", mu, "mol ",
                                              kg^-1, ")"))),
                       #breaks = seq(0, 10, 0.5),  
                       expand = c(0.05, 0.05)) +
    geom_point(aes (x=Date, y=n36TA, fill=Sub_region), 
             alpha=0.8, size=0.8, shape=21)

TA_all+
    geom_smooth(aes (x=Date, y=n36TA), method = "lm",
              color="black") + scale_fill_hue(l=100)

Trends_c<-TA_all+facet_grid(~Zone) +
  geom_smooth(aes (x=Date, y=n36TA, 
                   colour=Sub_region), 
              method = "lm", alpha=0.3)
Trends_c
...
```{r, echo=TRUE, fig.height=4, fig.width=6}
## Model with all data
TA_all_o<-lmer(n36TA~Year * Zone * Sub_region + (1|Month)+           
                                    (1|SiteID), data=Trends.data)
  step(TA_all_o)
    
    
TA_all_1<-lmer(n36TA ~ Year + Zone + Sub_region + 
                  (1 | Month) + (1 | SiteID) + 
                  Zone:Sub_region, data=Trends.data)
...
```{r, fig.height=4, fig.width=6}
# Inshore TA
  hist(Trends.inshore$n36TA)
  shapiro.test(Trends.inshore$n36TA) # Normality NOT rejected

  TA_in_0<-lmer(n36TA~Year * Sub_region + (1|Month)+
                                    #(1|Zone)+             
                                    (1|SiteID), data=Trends.inshore)
  #summary(TA_in_0)
  step(TA_in_0)
    
  TA_in<-lmer(n36TA ~ Year + Sub_region +
               (1 | Month) + (1 | SiteID),
                            data=Trends.inshore)
    anova(TA_in)
    ranova(TA_in)
  ...
  m.lst <- lstrends(TA_in, "Sub_region", var="Year")
  m.lst
  #pairs(m.lst)
...
# Model summary for labels
  slope<-round(summary(TA_in)$coefficients["Year", "Estimate"], digits = 3)
  ys <- max(Trends.midchanel$n36TA, na.rm =T)
  plot_label <- sprintf("\"\" ~ TA == %0.3f", slope)
  plot_label2 <- expression("Year^-1")  
...
```{r, fig.height=4, fig.width=6}
hist(Trends.midchanel$n36TA)
shapiro.test(Trends.midchanel$TA_umol_kg)# Normality rejected

TA_mc_0<-lmer(n36TA~Year * Sub_region + (1|Month) + 
                                  #(1|Zone)+             
                                  (1|SiteID), data=Trends.midchanel)
  anova(TA_mc_0)
  ranova(TA_mc_0)
...
TA_mc<-lmer(n36TA ~ Year + Sub_region+ (1 | Month) + (1|SiteID),
               data = Trends.midchanel)
  summary(TA_mc) 
  anova(TA_mc)
...
```{r, fig.height=4, fig.width=6}
hist(Trends.offshore$n36TA)
shapiro.test(Trends.offshore$TA_umol_kg)# Normality rejected

TA_of_0<-lmer(n36TA~Year * Sub_region + 
               (1|Month) + (1|SiteID),
               data=Trends.offshore)
  #summary(TA_of_0)    
  anova(TA_of_0)
...
# Create a new data frame for independent variables  
  ...
  #xs <- as.Date(as.character("2018-11-20"))
  #ys <- max(Trends.inshore$TA_umol_kg, na.rm =T)
  ys <- max(Trends.offshore$n36TA, na.rm =T)-100
  plot_label <- sprintf("\"\" ~ TA == %0.3f", slope)
  plot_label2 <- expression("Year^-1")
...
```{r, fig.height=4, fig.width=6}
hist(Trends.ocean$n36TA)
shapiro.test(Trends.ocean$n36TA)# Normality NOT rejected

TA_oc_0<-lmer(n36TA ~ Year * Sub_region + 
               (1 | Month)+ (1|SiteID),
               data=Trends.ocean)
  #summary(TA_oc_0)
  anova(TA_oc_0)
...
TA_oc<-lmer(n36TA ~ Year + (1 | Month), data = Trends.ocean)
  summary(TA_oc)
  anova(TA_oc)
...
# Model summary for labels
  slope<-round(summary(TA_oc)$coefficients["Year", "Estimate"], digits = 3)
  #xs <- as.Date(as.character("2018-11-20"))
  #ys <- max(Trends.inshore$n36TA, na.rm =T)
  ys <- max(Trends.offshore$n36TA, na.rm =T)
  plot_label <- sprintf("\"\" ~ TA == %0.3f", slope)
  plot_label2 <- expression("Year^-1")
...
```{r , all fixed factors summary - n36TA}
# ANOVAs
    # Convert ANOVA results into dataframes allows for easier name manipulation
    av.TA_in_0 <- data.frame (anova(TA_in_0))
    av.TA_mc_0 <- data.frame (anova(TA_mc_0) )
...
# R-ANOVAs    
    ...
    # create HTML table using kableExtra
    ranova_TA_0 %>% kable("html", digits=2,
         caption="Initial n36TA models random factors: Month+Station") %>% 
      kable_styling(bootstrap_options = "striped", full_width = F) %>% 
      pack_rows(., "TA inshore", 1, 3) %>% # groups rows with label
      pack_rows(., "TA mid-channel", 4, 6)  %>% # groups rows with label
      pack_rows(., "TA offshore", 7, 9) %>% # groups rows with label
...
modelsummary(TA_models_0, stars = TRUE, 
             #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Initial n36TA model outputs (all factors)',
             coef_map=factor_order)
#modelsummary(TA_models, estimate = "p.value")
```
...
modelsummary(TA_models, stars = TRUE, 
            #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Final n36TA model outputs (dropped factors)',
             coef_map=factor_order)
...
```{r n36TA model plot, fig.height=4, fig.width=8}
Trends_n36TA<-ggarrange(In_TA_Modeled+ theme(
                                          axis.text.x=element_blank(),
                                          legend.position = "none"),
                  Mc_TA_Modeled+ theme(legend.position = "none",
                                           axis.text.x=element_blank(),
...
```{r, echo=FALSE, fig.height=4, fig.width=8}
hist(Trends.data$n36DIC)
shapiro.test(Trends.data$n36DIC) # Normality rejected
...
DIC_all<- Long_plot+
    scale_y_continuous(#limits = c(15,40),
                       ...
                       #breaks = seq(0, 10, 0.5),  
                       expand = c(0.05, 0.05)) +
    geom_point(aes (x=Date, y=n36DIC, fill=Sub_region), 
             alpha=0.8, size=0.8, shape=21)

DIC_all+
    geom_smooth(aes (x=Date, y=n36DIC), method = "lm",
              color="black") + scale_fill_hue(l=100)

Trends_c<-DIC_all+facet_grid(~Zone) +
  geom_smooth(aes (x=Date, y=n36DIC, 
                   colour=Sub_region), 
              method = "lm", alpha=0.3)
Trends_c
...
```{r, echo=TRUE, fig.height=4, fig.width=6}
## Model with all data
DIC_all_o<-lmer(n36DIC~Year * Zone * Sub_region + (1|Month)+           
                                    (1|SiteID), data=Trends.data)
  step(DIC_all_o)
    
    
DIC_all_1<-lmer(n36DIC ~ Year + Zone + Sub_region + 
                  (1 | Month) + (1 | SiteID) + 
                  Year:Sub_region + Zone:Sub_region, data=Trends.data)
...
```{r, fig.height=4, fig.width=6}
# Inshore DIC
  hist(Trends.inshore$n36DIC)
  shapiro.test(Trends.inshore$DIC_umol_kg) # Normality NOT rejected

  DIC_in_0<-lmer(n36DIC~Year * Sub_region + (1|Month)+
                                    #(1|Zone)+             
                                    (1|SiteID), data=Trends.inshore)
  ...
  DIC_in<-lmer(n36DIC ~ Year + Sub_region +
               (1 | Month) + (1 | SiteID),
                            data=Trends.inshore)
    anova(DIC_in)
    ranova(DIC_in)
...
# Create a new data frame for independent variables  
  ...
  In_DIC_Modeled<-Long_plot2+ 
     scale_y_continuous(limits = c(1700,2320),
                       name=(expression(paste("n36DIC (", mu, "mol ",
                                              kg^-1, ")"))),
                       breaks = seq(1700,2320, 200),  
                       expand = c(0.02, 0.02)) +
...
```{r, fig.height=4, fig.width=6}
hist(Trends.midchanel$n36DIC)
shapiro.test(Trends.midchanel$DIC_umol_kg)# Normality rejected

DIC_mc_0<-lmer(n36DIC~Year * Sub_region + (1|Month) + 
                                  #(1|Zone)+             
                                  (1|SiteID), data=Trends.midchanel)
...
DIC_mc<-lmer(n36DIC ~ Year + Sub_region + (1 | Month),
               data = Trends.midchanel)
  summary(DIC_mc) 
  anova(DIC_mc)
...
```{r, fig.height=4, fig.width=6}
hist(Trends.offshore$n36DIC)
shapiro.test(Trends.offshore$DIC_umol_kg)# Normality rejected

DIC_of_0<-lmer(n36DIC~Year * Sub_region + 
               (1|Month) + (1|SiteID),
               data=Trends.offshore)
#summary(DIC_of_0)    
anova(DIC_of_0)
...
DIC_of<-lmer(n36DIC ~ Year + Sub_region + (1 | Month), 
               data =Trends.offshore)
  summary(DIC_of)
  anova(DIC_of) 
...
```{r, fig.height=4, fig.width=6}
hist(Trends.ocean$n36DIC)
shapiro.test(Trends.ocean$DIC_umol_kg)# Normality NOT rejected

DIC_oc_0<-lmer(n36DIC ~ Year * Sub_region + 
               (1 | Month)+ (1|SiteID),
               data=Trends.ocean)
  #summary(DIC_oc_0)
  anova(DIC_oc_0)
...
DIC_oc<-lmer(n36DIC ~ Year + (1 | Month), data = Trends.ocean)
  summary(DIC_oc)
  anova(DIC_oc)
...
```{r , all fixed factors summary - n36DIC}
# ANOVAs
    # Convert ANOVA results into dataframes allows for easier name manipulation
    av.DIC_in_0 <- data.frame (anova(DIC_in_0))
    av.DIC_mc_0 <- data.frame (anova(DIC_mc_0) )
    ...
    # create HTML table using kableExtra
    anova_DIC_0 %>% kable("html", digits=2, 
        caption="Initial n36DIC models fixed factors: Year * Region") %>% 
      kable_styling(bootstrap_options = "striped", full_width = F) %>% 
      pack_rows(., "DIC inshore", 1, 3) %>% # groups rows with label
      pack_rows(., "DIC mid-channel", 4, 6)  %>% # groups rows with label
      pack_rows(., "DIC offshore", 7, 9) %>% # groups rows with label
...
# R-ANOVAs    
    ...
    ranova_DIC_0 %>% kable("html", digits=2,
         caption="Initial DIC models random factors: Month+Station") %>% 
      kable_styling(bootstrap_options = "striped", full_width = F) %>% 
      pack_rows(., "n36DIC inshore", 1, 3) %>% # groups rows with label
      pack_rows(., "n36DIC mid-channel", 4, 6)  %>% # groups rows with label
      pack_rows(., "n36DIC offshore", 7, 9) %>% # groups rows with label
      pack_rows(., "n36DIC ocean", 10, 12)  
...
modelsummary(DIC_models_0, stars = TRUE, 
             #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Initial n36DIC model outputs (all factors)',
             coef_map=factor_order)
#modelsummary(DIC_models, estimate = "p.value")
```
...
modelsummary(DIC_models, stars = TRUE, 
            #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Final n36DIC model outputs (dropped factors)',
             coef_map=factor_order)
...
```{r n36DIC model plot, fig.height=4, fig.width=8}
Trends_n36DIC<-ggarrange(In_DIC_Modeled+ theme(
                                          #axis.text.x=element_blank(),
                                          legend.position = "none"),
                  Mc_DIC_Modeled+ theme(legend.position = "none",
                                           #axis.text.x=element_blank(),
...
All zones in one model per variable

```{r}
# Omega  
Arg_D<-lmer(Trends.data$Aragonite_Sat_W ~Year * Zone +
                     (1|Month) + (1|SiteID) + (1|Sub_region), 
                    data=Trends.data)
  #anova(Arg_D)
  #ranova(Arg_D)
  summary(Arg_D)
  lstrends(Arg_D, "Zone", var="Year")
  pairs(lstrends(Arg_D, "Zone", var="Year"))

# TA contribution model  
Arg_TA_in_f<-lmer(Arg_TA~Year*Zone + (1|Month) + (1|SiteID), 
                    data=Trends.data)
  summary(Arg_TA_in_f)
  lstrends(Arg_TA_in_f, "Zone", var="Year")
  pairs(lstrends(Arg_TA_in_f, "Zone", var="Year"))
  
# DIC contribution model
 Arg_DIC_in_f<-lmer(Arg_DIC~Year*Zone + (1|Month) + (1|SiteID), 
                    data=Trends.data)
  summary(Arg_DIC_in_f)
  lstrends(Arg_DIC_in_f, "Zone", var="Year")
  pairs(lstrends(Arg_DIC_in_f, "Zone", var="Year"))

# T contribution model    
Arg_Tem_in_f<-lmer(Arg_Tem ~Year*Zone + (1|Month) + (1|SiteID), 
                    data=Trends.data)
  summary(Arg_Tem_in_f)
  lstrends(Arg_Tem_in_f, "Zone", var="Year")
  pairs(lstrends(Arg_Tem_in_f, "Zone", var="Year"))
  
# S contribution model  
Arg_Sal_in_f<-lmer(Arg_Sal ~Year*Zone + (1|Month) + (1|SiteID), 
                    data=Trends.data)
  summary(Arg_Sal_in_f)
  lstrends(Arg_Sal_in_f, "Zone", var="Year")
  pairs(lstrends(Arg_Sal_in_f, "Zone", var="Year"))
```

Omega decomposition summary 

```{r}
Arg_Con_models<-list("dArg" = Arg_D,
                     "TA_Ar" = Arg_TA_in_f, 
                     "DIC_Ar" = Arg_DIC_in_f, 
                     "T_Ar" = Arg_Tem_in_f, 
                     "S_Ar" = Arg_Sal_in_f
...
modelsummary(Arg_Con_models, stars = TRUE, 
             #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Contribution to Aragonite Saturation State',
             coef_map=factor_order)
#modelsummary(TA_models, estimate = "p.value")
```
...
```{r}
Newdata_f<-expand.grid(Zone=unique(Trends.data$Zone),
                       Year=seq((min(Trends.data$Year)),
                                  (max(Trends.data$Year))))

Ar_Cal <- predict(Arg_D, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)
Ar_Cal<-Ar_Cal-mean_Op$OAra

ArTA_Pred_f <- predict(Arg_TA_in_f, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)
ArDIC_Pred_f <- predict(Arg_DIC_in_f, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)
ArTemp_Pred_f <- predict(Arg_Tem_in_f, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)
ArSal_Pred_f <- predict(Arg_Sal_in_f, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)
  
Ar_Pred_f <-cbind(Newdata_f, ArTA_Pred_f, ArDIC_Pred_f, 
                     ArTemp_Pred_f, ArSal_Pred_f, Ar_Cal)
Ar_Pred_f$Date<-as.Date(as.character(Ar_Pred_f$Year), format = "%Y")
Ar_Pred_f$Combined<-Ar_Pred_f$ArTA_Pred_f + Ar_Pred_f$ArDIC_Pred_f +
                    Ar_Pred_f$ArTemp_Pred_f + Ar_Pred_f$ArSal_Pred_f

# Plot raw data point and fitted model 
  Arg_Modeled<-Long_plot2 + facet_grid(~Zone)+
     geom_hline(yintercept = 0, colour="gray")+
     scale_color_manual(values = colors)+
     Zone_shapes4+
     scale_y_continuous(limits = c(-1,1),
                       ...
                       #breaks = seq(0, 10, 0.5),  
                       expand = c(0.02, 0.02)) +
          geom_line(data=Ar_Pred_f, aes(x=Date, y=ArTA_Pred_f, color="TA"),
                linetype=1, size=1)+
          geom_line(data=Ar_Pred_f, aes(x=Date, y=ArDIC_Pred_f, color="DIC"),
                linetype=1, size=1)+
          geom_line(data=Ar_Pred_f, aes(x=Date, y=ArTemp_Pred_f, color="Temperature"),
                linetype=1, size=1)+
          geom_line(data=Ar_Pred_f, aes(x=Date, y=ArSal_Pred_f, color="Salinity"),
                linetype=1, size=1)+
          geom_line(data=Ar_Pred_f, aes(x=Date, y=Combined, color="Combined"),
                linetype=1, size=1)+
          geom_line(data=Ar_Pred_f, aes(x=Date, y=Ar_Cal, color="Calculated"),
                linetype=3, size=1)#+
  Arg_Modeled
```
...
```{r}
# pH model
pH_D<-lmer(pH_calculated ~Year * Zone +
                     (1|Month) + (1|SiteID), 
                    data=Trends.data)
  summary(pH_D)
  lstrends(pH_D, "Zone", var="Year")
  pairs(lstrends(pH_D, "Zone", var="Year"))

# TA contribution model  
pH_TA_in_f<-lmer(pH_TA~Year*Zone + (1|Month) + (1|SiteID), 
                    data=Trends.data)
  summary(pH_TA_in_f)
  lstrends(pH_TA_in_f, "Zone", var="Year")
  pairs(lstrends(pH_TA_in_f, "Zone", var="Year"))

# DIC contribution model   
 pH_DIC_in_f<-lmer(pH_DIC~Year*Zone + (1|Month) + (1|SiteID), 
                    data=Trends.data)
  summary(pH_DIC_in_f)
  lstrends(pH_DIC_in_f, "Zone", var="Year")
  pairs(lstrends(pH_DIC_in_f, "Zone", var="Year"))

# T contribution model 
pH_Tem_in_f<-lmer(pH_Tem ~Year*Zone + (1|Month) + (1|SiteID), 
                    data=Trends.data)
  summary(pH_Tem_in_f)
  lstrends(pH_Tem_in_f, "Zone", var="Year")
  pairs(lstrends(pH_Tem_in_f, "Zone", var="Year"))

# S contribution model  
pH_Sal_in_f<-lmer(pH_Sal ~Year*Zone + (1|Month) + (1|SiteID), 
                    data=Trends.data)
  summary(pH_Sal_in_f)
  lstrends(pH_Sal_in_f, "Zone", var="Year")
  pairs(lstrends(pH_Sal_in_f, "Zone", var="Year"))
```

Omega decomposition summary

```{r}
pH_Con_models<-list( "dpH" = pH_D,
                     "TA_pH" = pH_TA_in_f, 
                     "DIC_pH" = pH_DIC_in_f, 
                     "T_pH" = pH_Tem_in_f, 
                     "S_pH" = pH_Sal_in_f
...
modelsummary(pH_Con_models, stars = TRUE, 
             #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Contribution to pH)',
             coef_map=factor_order)
#modelsummary(TA_models, estimate = "p.value")
```
...
Newdata_f<-expand.grid(Zone=unique(Trends.data$Zone),
                        Year=seq((min(Trends.data$Year)),
                                   (max(Trends.data$Year))))
pH_Cal <- predict(pH_D, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)
pH_Cal<-pH_Cal-mean_Op$OpH

pHTA_Pred_f <- predict(pH_TA_in_f, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)
pHDIC_Pred_f <- predict(pH_DIC_in_f, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)
pHTemp_Pred_f <- predict(pH_Tem_in_f, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)
pHSal_Pred_f <- predict(pH_Sal_in_f, newdata=Newdata_f, 
                         re.form=NA, level=0, asList = FALSE)

pH_Pred_f <-cbind(Newdata_f, pHTA_Pred_f, pHDIC_Pred_f, 
                     pHTemp_Pred_f, pHSal_Pred_f, pH_Cal)
pH_Pred_f$Date<-as.Date(as.character(pH_Pred_f$Year), format = "%Y")

pH_Pred_f$Combined<-pH_Pred_f$pHTA_Pred_f + pH_Pred_f$pHDIC_Pred_f +
                    pH_Pred_f$pHTemp_Pred_f + pH_Pred_f$pHSal_Pred_f
...
# Plot raw data point and fitted model 
  pH_Modeled<-Long_plot2 + facet_grid(~Zone)+
     ...
     scale_y_continuous(limits = c(-0.10,0.1),
                       ...
                       #breaks = seq(0, 10, 0.5),  
                       expand = c(0.0, 0.0)) +
      geom_line(data=pH_Pred_f, aes(x=Date, y=pHTA_Pred_f, color="TA"),
                linetype=1, size=1)+
          geom_line(data=pH_Pred_f, aes(x=Date, y=pHDIC_Pred_f, color="DIC"),
                linetype=1, size=1)+
          geom_line(data=pH_Pred_f, aes(x=Date, y=pHTemp_Pred_f, color="Temperature"),
                linetype=1, size=1, alpha=0.8)+
          geom_line(data=pH_Pred_f, aes(x=Date, y=pHSal_Pred_f, color="Salinity"),
                linetype=1, size=1)+
  geom_line(data=pH_Pred_f, aes(x=Date, y=Combined, color="Combined"),
                linetype=1, size=1)+
  geom_line(data=pH_Pred_f, aes(x=Date, y=pH_Cal, color="Calculated"),
                linetype=3, size=1)
pH_Modeled
```

# - Figure 7: Contribution of TA, DIC, S and T to omega and pH interannual trends

```{r}
Sensitivity<-ggarrange(Arg_Modeled+ theme(axis.title.x=element_blank(),
                                   axis.text.x=element_blank(),
                                   legend.position = "top")+
                                   guides(colour = guide_legend(nrow = 1)),
                       Cont_arg_plot + theme(axis.title.x=element_blank(),
                                   axis.text.x=element_blank(),
                                   axis.title.y=element_blank(),
                                   legend.position = "top")+
                                   guides(colour = guide_legend(nrow = 1)),
                       pH_Modeled + theme(legend.position = "none",
                                        strip.text.x = element_blank(),
                       ...
                       Cont_pH_plot + theme(legend.position = "none",
                                            axis.title.y=element_blank(),
                                       strip.text.x = element_blank()),
          ncol = 2, nrow = 2, heights  =c(1.25, 1, 1,1), 
                              widths = c(1, 1, 1, 1))
Sensitivity
#ggsave(file="FLK_results/Figures/Fig_7/Fig_7_Contributions.svg", plot=Sensitivity, dpi = 300, width=7.5, height=4.5)
```

**Figure 7: Drivers of seasonal variability in ΩAra and pH in the Florida Reef.** Contributions of changing TA (purple), DIC (green), temperature (red), and salinity (blue) to changing aragonite saturation state and pH with respect to mean oceanic values. The black lines represent the combined effects estimated from TA, DIC, S and T. The gray lines represent the calculated ΩAra and pH from the in-situ values. 
