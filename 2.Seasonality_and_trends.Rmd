...
```{r}
Cont_arg_plot_TA<- ggplot(FLK.data_filtered,
                          aes(x=Months, y=Arg_TA, fill=Sub_region)) +
        MyTheme +  
        facet_grid(~Zone)+
        Zone_shapes4+
        ...
        scale_y_continuous(#limits = c(-2.5,2.5),
                       name=(expression(
                        (delta~Omega[Ar]~delta~TA^-1)~Delta~TA)),
                       breaks = seq(-2, 2, 1),  
                       expand = c(0.01, 0.01))+
...
Cont_arg_plot_DIC<- ggplot(FLK.data_filtered, aes(x=Months, y=Arg_DIC, fill=Sub_region)) +
        MyTheme +  
        facet_grid(~Zone)+
        geom_hline(yintercept = 0, colour="gray")+
        geom_smooth(aes (x=as.numeric(Months),
...
Cont_arg_plot_Sal<- ggplot(FLK.data_filtered, aes(x=Months, y=Arg_Sal, fill=Sub_region)) +
        MyTheme +  
        facet_grid(~Zone)+
        Zone_shapes4+
        geom_hline(yintercept = 0, colour="gray")+
...
Cont_arg_plot_Temp<- ggplot(FLK.data_filtered, aes(x=Months, y=Arg_Tem, fill=Sub_region)) +
        MyTheme +  
        facet_grid(~Zone)+
        geom_hline(yintercept = 0, colour="gray")+
        geom_smooth(aes (x=as.numeric(Months),
                      y=Arg_Temp, colour=Sub_region),
                span=0.8, alpha=0.2, se=F)+
        Zone_shapes4+
        stat_summary(aes (x=as.numeric(Months),
                          y=Arg_Temp, colour=Sub_region),
                    ...
                    position=position_dodge(width=0.3))+
        stat_summary(aes (x=as.numeric(Months), 
                          y=Arg_Temp, fill=Sub_region, shape=Zone),
                    size=2,
                    fun.data = "mean_cl_boot", geom = "point",
                    position=position_dodge(width=0.3))+
        scale_y_continuous(#limits = c(-2.5,2.5),
...
Omega_cont_plot<-ggarrange(Cont_arg_plot_TA+ theme(
          ...
          ncol = 1, nrow = 4, heights = c(1.4, 0.9, 0.9, 1.1))
Omega_cont_plot

```

* TA+DIC and S+T contributions by region, zone and variable to omeaga
...
Cont_arg_plot<- ggplot(FLK.data_filtered) +
        ...
        geom_smooth(aes (x=as.numeric(Months),
                      ...
                      color="Calculated"),
                span=0.8, alpha=0.2, se=F, linetype=2)+
        scale_y_continuous(#limits = c(-2.5,2.5),
                       name=(expression(
...
TA_mc<-lmer(TA_umol_kg ~ Sub_region + (1 | Month) + (1|SiteID),
               data = Trends.midchanel)
  summary(TA_mc) 
  anova(TA_mc)
...
Sensitivity<-ggarrange(Cont_arg_plot+ theme(axis.title.x=element_blank(),
                       ...
                       Arg_Modeled + theme(axis.title.x=element_blank(),
                                   axis.text.x=element_blank(),
                                   axis.title.y=element_blank(),
                                   legend.position = "top")+
                                   guides(colour = guide_legend(nrow = 1)),
